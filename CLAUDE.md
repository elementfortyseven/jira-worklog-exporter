...
| `jwe.gui` | ✅ stages 1-5b + Stage 6 + JWE-34 + JWE-49 + JWE-50 + JWE-35 + JWE-36 + JWE-37 complete | Full GUI implementation: AuthWidget with dual-mode panels, UserSearchWidget with debounced search, FilterWidget, OutputWidget, StatusWidget with progress + cancel + result buttons; ExportWorker and UserSearchWorker via Pattern C (persistent worker threads with lazy start); closeEvent confirmation; QSettings round-trip for all persistent fields; full i18n (two-channel model, runtime language switch, QSettings persistence). Frameless window shell (JWE-34): FramelessWindowHint + WA_TranslucentBackground, #windowFrame with drop shadow, custom TitleBar (brand, DE/EN segmented toggle, min/max/close), edge resize via startSystemResize, manual maximize/restore with availableGeometry. JWE-49: resize hot-zone against outer rect + slim 10px shadow margin. JWE-50: native Win32 WM_NCHITTEST/WM_NCCALCSIZE via nativeEvent on Windows; DWM shadow + Win11 rounded corners; startSystemResize/Move fallback retained for non-Windows. Pure geometry helper _nc_hit_region tested offscreen (15 unit tests). JWE-35: SectionCard(QFrame) wraps all four sections (QGroupBox removed); icon badges (QPainter-drawn), [ NN ] index, title/subtitle, right-slot, accent tick; auth mode selector in auth card head-end; new section subtitle i18n keys. JWE-36: form-control restyle -- input hover/disabled/invalid states, flat QComboBox drop-down + dark popup list, QPainter eye-toggle on token fields, QRadioButton segmented pill (authModeBtn/authModePill), ghost button variant, button mono font, distinct disabled states, QCalendarWidget dark theme, [invalid="true"] error border on auth/filter/output fields with signal-loop-safe re-polish, close-X white-on-hover via eventFilter, Discover/Browse fixed-width nits removed. JWE-37: identity strip + status chip. New `widgets/chip.py` (Chip(QFrame), objectName "chip", `set_state(variant, label)` -- dynamic `variant` property ok/error/testing/neutral, uppercased label, solid dot for ok, static non-animated spinner ring for testing, re-polish on change). New `widgets/identity_strip.py` (IdentityStrip(QWidget), objectName "idStrip", hidden by default; `set_identity(name, account_id)` shows a "Connected as {name}" bar with tooltip carrying the full accountId; click copies the full accountId to the clipboard and flashes a "Copied" confirmation for 1200ms via QTimer.singleShot; `clear()` hides it). `ConnectionTestWorker.finished` widened to `Signal(str, str, str)` (display_name, email, account_id) -- accountId was already returned by `service.test_connection()`'s `User`, only the worker/signal needed to forward it. `AuthWidget` gained `identity_verified = Signal(str, str)`, wires `conn_chip` (testing on click, ok on success, error on failure) and blanks `status_label` on success (the connected-as text now lives in the strip). `MainWindow` inserts `IdentityStrip` between the title bar and the scroll body, wired to `identity_verified`/`connection_invalidated`. `app.qss` gains `#idStrip`/`#chip[variant=...]` rules; no ok/err tint-alpha or dot-glow tokens exist yet (flagged for JWE-55), so the ok/error chip variants use a solid border+text color with a transparent background rather than a tinted fill. 990 tests green, 5 skipped. |
...
**JWE-36 delivered (2026-06-29):** Form-control restyle (absorbs JWE-1/JWE-3). `app.qss`: button base rule gains `font-family: %(font.mono)s; font-size: 12px; font-weight: 500;`; added `:hover` (line.strong border), `:disabled` (visually distinct bg/border/color), and re-asserted `:focus` after hover so accent border wins when focused; `QLineEdit[invalid="true"]` / `QComboBox[invalid="true"]` / `QDateEdit[invalid="true"]` / `QSpinBox[invalid="true"]` error border via `%(status.err)s`; flat `QComboBox::drop-down` / `QDateEdit::drop-down` (transparent, no separator); `QComboBox QAbstractItemView` dark popup (bg.card, hairline border, accent selected, rgba hover reusing existing literal); `QPushButton#ghostBtn` variant (transparent bg, same hover/disabled); `QPushButton:disabled` distinct state; `QFrame#authModePill` + `QRadioButton#authModeBtn` segmented pill (indicator hidden, border-radius 999px, checked -> accent fill + accent.ink); `QCalendarWidget` dark theme (nav bar, cells, selected day, hover). `auth.py`: QPainter-drawn eye/eye-off `QIcon` (`_eye_icon`, DPI-aware via `_screen_dpr`); `QLineEdit.addAction(icon, TrailingPosition)` on both SA and User token fields; `_toggle_token_visibility()` toggles echoMode + updates icon; `QButtonGroup` pill wraps the two `QRadioButton#authModeBtn` radios; `auth_mode_selector` remains the pill `QFrame`; `_update_invalid_state()` marks required fields `[invalid]` when active panel has empty inputs, with guard `if field.property("invalid") != inv` to prevent re-polish loops; `_on_field_changed()` calls `_update_invalid_state()` before emitting `validation_changed`; `discover_btn.setFixedWidth(80)` removed. `filter.py`: `_on_field_changed()` + `_update_invalid_state()` for inverted date range and bad project-key format. `output.py`: `_on_dir_changed()` + `_update_invalid_state()` for non-empty nonexistent path; `browse_btn.setFixedWidth(80)` removed. `title_bar.py`: `eventFilter` on `win_close_btn` (`WA_Hover` + `installEventFilter`) swaps close icon to white on `HoverEnter`, back to SECONDARY on `HoverLeave`. All colours routed through tokens (no new rgba literals beyond one reused existing value). 33 new tests (TestSegmentedControl, TestPasswordEye, TestInvalidProperty across auth/filter/output); full suite: 951 passed, 5 skipped.

**JWE-37 delivered (2026-08-31):** Identity strip + reusable status chip, with accountId pass-through from `/myself`. New `jwe/gui/widgets/chip.py`: `Chip(QFrame)`, objectName `chip`, `set_state(variant, label)` sets a `variant` dynamic property (`ok`/`error`/`testing`/`neutral`), uppercases the label, shows a solid dot for `ok` and a static (non-animated) spinner ring for `testing` -- real animation is JWE-40 -- then re-polishes. New `jwe/gui/widgets/identity_strip.py`: `IdentityStrip(QWidget)`, objectName `idStrip`, hidden by default; `set_identity(name, account_id)` shows `t("idstrip.connected_as", lang, name=name)` with a tooltip carrying the full accountId + copy hint; `mousePressEvent` copies the full accountId to `QApplication.clipboard()` and flashes `t("idstrip.copied")` for 1200ms via `QTimer.singleShot` before reverting; `clear()` hides it. `ConnectionTestWorker.finished` widened from `Signal(str, str)` to `Signal(str, str, str)` (display_name, email, account_id) -- `service.test_connection()` already returned a `User` with `account_id`, so only the worker's emit needed to forward it; no `service.py` change was required. `AuthWidget` gained `identity_verified = Signal(str, str)` (display_name, account_id); `conn_chip` (a `Chip`) sits next to `test_btn` and moves testing -> ok/error across the connection-test lifecycle; `status_label` is blanked on success (the connected-as text now lives in the strip) and still carries the error message on failure. `MainWindow` inserts `identity_strip` into `frame_layout` between the title bar and the scroll body; wires `auth_widget.identity_verified` -> `identity_strip.set_identity` and `auth_widget.connection_invalidated` -> `identity_strip.clear`; `_retranslate_all` calls `identity_strip.retranslate_ui(lang)`. `app.qss` gains `#idStrip` (qlineargradient accent-tint bg reusing the existing `rgba(34,211,238,0.08)` literal, `line.soft` border-bottom) and `#chip[variant="ok"|"error"|"testing"]` (solid border + text color via `status.ok`/`status.err`/`accent.base`; the CSS prototype's tinted `chip--ok`/`chip--err` backgrounds and dot glow have no alpha token anywhere -- not in `tokens.py`, not in `docs/design/tokens.json` -- so they are omitted rather than adding new literals; flagged for JWE-55). New i18n keys (DE+EN): `chip.testing`, `chip.connected`, `chip.error`, `idstrip.connected_as`, `idstrip.copy_hint`, `idstrip.copied`. 33 net new tests (14 `Chip`, 10 `IdentityStrip`, 5 net new/reworked `AuthWidget` connection-test/chip/identity-signal cases including two negative cases, 4 `MainWindow` wiring); full suite: 990 passed, 5 skipped.
